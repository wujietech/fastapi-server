...
import { WorkflowPublic, WorkflowService } from "@/client"
...
function getItemsQueryOptions({ pageNumber }: { pageNumber: number }) {
  return {
    queryFn: () =>
      WorkflowService.readWorkflows({ pageNumber, pageSize: PER_PAGE }),
...
function WorkflowActionsMenu({ workflow }: { workflow: WorkflowPublic }) {
  return (
    <Flex gap={2}>
      <Button
        leftIcon={<FiEdit />}
        variant="ghost"
        size="sm"
        onClick={() => console.log('编辑', workflow.id)}
      >
        编辑
      </Button>
      <Button
        ...
        variant="ghost"
        size="sm"
        onClick={() => console.log('删除', workflow.id)}
      >
        删除
      </Button>
    </Flex>
